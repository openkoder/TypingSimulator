...
#define DELAY_MIN 4000      // Minimum delay in microseconds
#define DELAY_MAX 40000     // Maximum delay in microseconds

// ERROR_PROB defines the probability of making a typing error (e.g., random character insertion)
#define ERROR_PROB 0.020     // Probability of making a typing error (2%)
...
void update_status_line() {
	...
	int current_lines = getmaxy(stdscr);  // Get the total number of lines in the terminal
...
void type_char(char c) {
	...
	int current_lines = getmaxy(stdscr); // Get the total number of lines in the terminal window
	...
	addch(c);  // Type the actual character
...
void print_highlighted(const char* code) {
	...
	while (*code) {
		...
		for (int i = 0; i < num_keywords; i++) {
			size_t len = strlen(keywords[i]);  // Get the length of the current keyword
		...
		if (!is_keyword) {  // If the current segment is not a keyword
			if (*code == '"') {  // Check if it's the start of a string literal
				do {
					color_pair = 3;
				...
				} while (*code && *code != '"');  // Continue until the end of the string literal
				color_pair = 3;
...
void read_code_from_file(const char* filename, char** buffer) {
	...
	*buffer = malloc(length + 1);
	...
	(*buffer)[length] = '\0';  // Add a null-terminator at the end of the buffer to make it a valid C string
...
int main() {
	...
	char* code = NULL;  // Pointer to hold the code read from the file
